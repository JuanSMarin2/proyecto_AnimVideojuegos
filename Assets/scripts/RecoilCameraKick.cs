...
namespace Assets.scripts
{
    public class RecoilCameraKick : MonoBehaviour
    {
        ...
        {
            StopAllCoroutines();
            StartCoroutine(routine: KickCoroutine(strength, peakDuration, recoverDuration));
        }
        ...
        IEnumerator KickCoroutine(float strength, float peak, float recover)
        {
            ...
            while (t < peak)
            {
                ...
                {
                    if (perlins[i]) perlins[i].AmplitudeGain = Mathf.Lerp(baseAmplitud[i], baseAmplitud[i] + strength, t:k);
                }
            ...
            while (t < recover) 
            {
                ...
                for(int i = 0; i < perlins.Length; i++)
                {
                    if (perlins[i]) perlins[i].AmplitudeGain = Mathf.Lerp(baseAmplitud[i] + strength, baseAmplitud[i], t: k);
                }
