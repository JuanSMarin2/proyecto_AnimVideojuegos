...
public class FootIKSimple : MonoBehaviour
{
    ...
    public float rayAdove = 0.6f, rayBelow = 1.0f;
    ...
    /*private void OnAnimatorIK(int layerIndex)
    {
        ...
        bool lHit = Physics.Raycast(origin:lPose+Vector3.up * rayAdove, direction:Vector3.down
            , out RaycastHit rHit, maxDistance:rayAdove + rayBelow, groundMask, QueryTriggerInteraction.Ignore);
        ...
        if (movePelvis && hips)
        {
            ...
            target = Mathf.Clamp(target*pelvisAdjust, min:-pelvisClamp, max:pelvisClamp);

            float newOffset = Mathf.SmoothDamp(current: pelvisOffset, target, currentVelocity: ref pelvisVel, pelvisSmooth);
